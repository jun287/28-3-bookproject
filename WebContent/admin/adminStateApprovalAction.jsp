...
<%
	...
	orders = serviceBookOrders.selectOrders(ordersNumber);					// 한 개 주문 조회
	
	int memberNumber = orders.getMemberNumber();							
	int bookNumber = orders.getBookNumber();
	int shoppingCartNumber = shoppingCart.getShoppingCartNumber();
	member = serviceMember.selectMemberPoint(memberNumber);					// 멤버 포인트 조회
		
	// book 테이블에서 book_point * orders 테이블에 orders_amount  = 적립 포인트
	Book book = new Book();
	serviceBookOrders.seslectBookOrders(bookNumber);						// BookOrder 조회
	...
	serviceBookOrders.selectforUpdateBookAmount(bookNumber, ordersNumber);		// bookNumber 와  ordersNumber 조회 후 bookAmount 업데이트

	serviceBookOrders.insertBookOrders(memberNumber, memberPoint, orders, shoppingCartNumber);

	// updateMemberPoint(memberNumber, memberPoint, connection);					// 주문 후 기존의 memberPoint 업데이트 
